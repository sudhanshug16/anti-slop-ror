...
def run!(*command, chdir:)
  output, status = Open3.capture2e({"BUNDLE_GEMFILE" => nil}, *command, chdir: chdir)
...
def verify_consumer!(target, config)
  ...
  output, status = Open3.capture2e({"BUNDLE_GEMFILE" => nil}, "bundle", "exec", "standardrb", "--config", ".standard.yml", "--raise-cop-error", "violation.rb", chdir: target)
